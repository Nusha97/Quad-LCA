...
def main():
    ...
    rospy.init_node('mav_planner_test', anonymous=True)
    ...
    for i in range(10):
        ...
        ref = generate_lissajous_traj(np.linspace(0, 3, 301), x_num_periods, y_num_periods, z_num_periods, yaw_num_periods, period, x_amp, y_amp, z_amp, yaw_amp)
        ...
        vf = valuefunc.MLPValueFunc(4214, [300, 300, 1])
        vf.network.load_state_dict(torch.load('/home/anusha/mlp_model_dict'))

        nn_coeff = quadrotor.generate(torch.tensor(waypt), ts, order, Tref, p, rho, vf, torch.zeros([4, 6, 6]).double(), num_iter=100, lr=0.005)

        pos, vel, acc, jerk, yaw, yaw_dot = compute_pos_vel_acc(Tref, nn_coeff, segments, ts)
