...
def main():
    ...
    coeff_obj = load_object(coeff_file)
    rospy.init_node('inference_grad', anonymous=True)
    ...
    for k in seeds:
        ...
        for j in range(20):
            #print("j", j)
            #print(len(coeff_obj[i]))
            init_coeff = coeff_obj[k][j].numpy()
            pos, vel, acc, jerk, yaw, yaw_dot = compute_pos_vel_acc(Tref, init_coeff, segments, ts)
