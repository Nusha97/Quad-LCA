...
def main():
    ...
    for i in range(num_traj):
        ...
        #ref_coeff, _, _, _, _, _ = compute_pos_vel_acc(horizon, init_coeff, segments, ts)
    ...
    for i in range(3):
        if i == 0:
            ref_traj_ours, actual_traj_ours, input_traj, cost_traj, times_init = load_infbags(
                '/home/anusha/replan/dragonfly1-nn0.bag',
                'dragonfly1',
                "/home/anusha/Research/ws_kr/src/layered_ref_control/src/layered_ref_control/data/times_nn0.pkl",
                0)
            #ref_traj, actual_traj, input_traj, cost_traj, times_init = load_infbags('/home/anusha/poly_bag2.bag',
            #                                                                            'dragonfly1',
            #                                                                            "/home/anusha/poly_ref_times.pkl",
            #                                                                            1)

            #ref_traj, actual_traj, input_traj, cost_traj, times_init = load_infbags('/home/anusha/dragonfly1-poly.bag',
            #                                                                        'dragonfly1',
            #                                                                        "/home/anusha/poly_inf_times.pkl",
            #                                                                        1)
            ...
            ref_traj_poly = ref_traj
            actual_traj_poly = actual_traj
            print("Shape", ref_traj.shape)
            print(actual_traj.shape)"""

        elif i == 1:
            ref_traj_poly, actual_traj_poly, input_traj, cost_traj, times_opt = load_infbags(
                '/home/anusha/replan/dragonfly1-poly.bag',
                'dragonfly1',
            ...
            ref_traj_ours = ref_traj
            actual_traj_ours = actual_traj
    ...
    for j in range(1):
        ...
        axes.plot3D(ref_traj_poly[:, 0], ref_traj_poly[:, 1], ref_traj_poly[:, 2], label="poly ref")
        #axes.plot3D(actual_traj_poly[:, 0], actual_traj_poly[:, 1], actual_traj_poly[:, 2], label="poly act")
        #axes.plot3D(ref_traj_ours[:, 0], ref_traj_ours[:, 1], ref_traj_ours[:, 2], label="our ref")
        #axes.plot3D(actual_traj_ours[:, 0], actual_traj_ours[:, 1], actual_traj_ours[:, 2], label="our act")
        """axes.plot3D(ref_traj_poly[:, j * 4], ref_traj_poly[:, j * 4+1],
                        ref_traj_poly[:, j * 4+2], 'r--', label="poly ref")
        #axes.plot3D(ref_traj_poly[:, 0], ref_traj_poly[:, 1],
        #            ref_traj_poly[:, 2], 'r--', label="poly ref")
        #axes.plot3D(np.array(ref_traj_poly[range(start[j], end[j]), 0]),
        #            np.array(ref_traj_poly[range(start[j], end[j]), 1]),
        #            np.array(ref_traj_poly[range(start[j], end[j]), 2]), 'r-', label="poly ref")
        axes.plot3D(np.array(ref_traj_ours[range(start[j*4], end[j*4]), 0]), np.array(ref_traj_ours[range(start[j*4], end[j*4]), 1]),
                    np.array(ref_traj_ours[range(start[j*4], end[j*4]), 2]), 'g-', label="our ref")
        axes.plot3D(np.array(actual_traj_poly[range(start[j], end[j]), 0]), np.array(actual_traj_poly[range(start[j], end[j]), 1]),
                    np.array(actual_traj_poly[range(start[j], end[j]), 2]), 'b--', label="poly sim")
        axes.plot3D(np.array(actual_traj_ours[range(start[j*4], end[j*4]), 0]), np.array(actual_traj_ours[range(start[j*4], end[j*4]), 1]),
                    np.array(actual_traj_ours[range(start[j*4], end[j*4]), 2]), 'y-', label="our sim")"""
        axes.set_xlim(-5, 10)
        axes.set_zlim(-5, 10)
        axes.set_ylim(-5, 10)
        ...
        axes.plot3D(waypt[0, :], waypt[1, :], waypt[2, :], '*', label="waypoints")
        #axes.plot3D(way_pt_all[j][0, :], way_pt_all[j][1, :], way_pt_all[j][2, :], '*', label="waypoints")
        plt.title("3D Visualization of the trajectories")
